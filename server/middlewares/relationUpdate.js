...
const relationUpdateMiddleware = async (ctx, next) => {
  try {
    const params = ctx.params || ctx.request?.params || {};
    ...
    const entry =
      (await strapi.db.query(model).findOne({ where: { documentId: id } })) ||
      (await strapi.db.query(model).findOne({ where: { id } }));
    ...
    const allVersionIdsNumbers = (allVersionIds || []).map((v) => v.id);
    if (allVersionIdsNumbers.length < 2) {
      // there are no multiple versions, no need to update relations
      return await next();
    }
  ...
  return await next();
    ...
    strapi.log.error(`[relationUpdateMiddleware error]: ${err.message}`, err);
    return await next();
  }
};
